...
export default function SignUpScreen({ navigation }: SignUpScreenProps) {
  ...
  const [nutricionista, setNutricionista] = useState<Usuario[]>([]);

  useEffect(() => {
    // Cargar entrenadores
    fetch("http://localhost:8081/api/users/entrenadores")
      .then((res) => res.json())
      .then((data) => setEntrenadores(data))
      .catch((err) => console.error("Error al cargar entrenadores:", err));

    // Cargar nutricionistas
    fetch("http://localhost:8081/api/users/nutricionistas")
      .then((res) => res.json())
      .then((data) => setNutricionista(data))
      .catch((err) => console.error("Error al cargar nutricionistas:", err));
  }, []);
  ...
  return (
    <ScrollView contentContainerStyle={styles.scrollContainer}>
      <View style={styles.container}>
        ...
        <View>
          <Text>Selecciona un entrenador:</Text>
          <Picker>
            {entrenadores.map((e) => (
              <Picker.Item key={e.dni} label={e.nombre} />
            ))}
          </Picker>

          <Text>Selecciona un nutricionista:</Text>
          <Picker>
            {nutricionista.map((n) => (
              <Picker.Item key={n.dni} label={n.nombre} />
            ))}
          </Picker>
        </View>
